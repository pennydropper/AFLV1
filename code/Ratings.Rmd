...
rtngs.plot.data %>%
  ...
  mutate(tm.abbr = fct_reorder2(tm.abbr, rnd, rating.tm),
         meas = factor(meas, labels = c("model", "actual"))) %>%
  ggplot(aes(x = meas, y = margin, fill = meas)) +
  scale_y_continuous(breaks = seq(-60, 60, 24)) +
  geom_rect(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf, 
            aes(fill = mod.corr), alpha = 0.25, show.legend = FALSE) +
  geom_hline(aes(yintercept = 0), colour = "black", alpha = 0.5) +
  geom_bar(stat = "identity") +
  scale_fill_manual("",
                      values = c("actual" = "#01665e", "model" = "#d8b365", "Corr" = "#5ab4ac", "Inc" = "white", "Draw" = "grey"),
                      labels = c("actual" = "actual", "model" = "pred", rep("", 3))) +
  coord_flip() +
  labs(x = "", y = "") +
